...
- Raw Codex `process/stderr` WebSocket connect/stream messages are diagnostics, not terminal provider failure. Keep them as `runtime.warning`; rely on structured Codex `error` notifications with `willRetry: false`, process exit, or terminal turn events before surfacing `runtime.error`.
