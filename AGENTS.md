...
- Upstream Codex TUI treats steer failures as active-turn reconciliation: if there is no active turn, it clears its cached active turn and starts a new turn; if the active turn is not steerable, it queues rejected input. Cafe Code should match that safety model by only sending `turn/steer` while its projection still has a live streaming assistant message for the active Codex turn. If the assistant stream has closed but `turn/completed` is delayed or missing, keep the follow-up queued or interrupt the stale active turn before starting a new turn; do not append a late steer into the stale turn.
