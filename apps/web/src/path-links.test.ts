...
describe("extractTerminalLinks", () => {
  it("finds http urls and path tokens", () => {
    const line = "failed at https://example.com/docs and src/components/ChatView.tsx:42";
    expect(extractTerminalLinks(line)).toEqual([
      ...
      {
        kind: "path",
        text: "src/components/ChatView.tsx:42",
        start: 39,
        end: 69,
      },
    ]);
...
describe("resolveWrappedTerminalLinkRange", () => {
  it("maps wrapped URL matches back to the correct buffer rows", () => {
    ...
    if (!wrappedLine) {
      throw new Error("Expected wrapped path line to be present.");
    }
...
describe("resolvePathLinkTarget", () => {
  it("resolves relative paths against cwd", () => {
    expect(resolvePathLinkTarget("src/components/ChatView.tsx:42:7", "/Users/julius/project")).toBe(
