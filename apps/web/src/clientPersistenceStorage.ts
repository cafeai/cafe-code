...
function documentContainsLegacyFields(
  document: LegacyBrowserSavedEnvironmentRegistryDocument,
): boolean {
  return (document.records ?? []).some(
    (record) => record.bearerToken !== undefined || record.desktopSsh !== undefined,
  );
}
...
function readBrowserSavedEnvironmentRegistryDocument(): BrowserSavedEnvironmentRegistryDocument {
  ...
  try {
    ...
    if (documentContainsLegacyFields(parsed)) {
