...
/**
 * Provider-native payloads can contain full prompts, model output, tool
 * arguments, filesystem paths, and authentication headers. Canonical events
 * already carry the safe fields the renderer needs, so raw ACP diagnostics
 * retain structure only and never duplicate sensitive content.
 */
function summarizeRawPayload(payload: unknown): Readonly<Record<string, unknown>> {
...
export function makeAcpRequestOpenedEvent(input: {
  ...
  return {
    ...
    raw: {
      source: input.source,
      method: input.method,
      payload: summarizeRawPayload(input.rawPayload),
    },
  };
}
...
export function makeAcpToolCallEvent(input: {
  ...
  return {
    ...
    payload: {
      ...
      // ACP tool updates contain the command, locations, and bounded result
      // previews that Cafe's existing work-log presentation understands. The
      // ingestion boundary sanitizes this value before persistence, so retain
      // the structured data here instead of reducing Grok to a generic title.
