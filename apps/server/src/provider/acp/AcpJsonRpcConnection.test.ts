    ...
    return Effect.gen(function* () {
      ...
      Effect.provide(
        AcpSessionRuntime.layer({
          ...
          clientInfo: { name: "cafecode-test", version: "0.0.0" },
          authMethodId: "test",
          requestLogger: (event) =>
            Effect.sync(() => {
      ...
      Effect.provide(
        AcpSessionRuntime.layer({
          ...
          spawn: {
            ...
            args: mockAgentArgs,
            env: {
              CAFE_CODE_ACP_FAIL_LOAD_SESSION: "1",
            },
          },
          cwd: process.cwd(),
  ...
  it.effect("completes session/load after replay becomes idle while its RPC stays pending", () =>
    Effect.gen(function* () {
      ...
      expect(started.sessionSetupResult._meta).toMatchObject({
        cafeCodeSessionLoadReady: "replay_idle",
      });
      ...
      Effect.provide(
        AcpSessionRuntime.layer({
          ...
          spawn: {
            ...
            args: mockAgentArgs,
            env: {
              CAFE_CODE_ACP_HANG_LOAD_SESSION_AFTER_REPLAY: "1",
              CAFE_CODE_ACP_LOAD_SESSION_DELAY_MS: "10000",
            },
          },
          cwd: process.cwd(),
          resumeSessionId: "mock-session-1",
      ...
      expect(
        requestEvents.some(
          (event) =>
            ...
            (event.payload as { readonly value?: unknown }).value === "composer-2[fast=false]",
        ),
      ).toBe(false);
    }).pipe(
      Effect.provide(
