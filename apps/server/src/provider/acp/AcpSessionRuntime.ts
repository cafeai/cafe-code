...
export interface AcpSessionRuntimeOptions {
  ...
  /**
   * Selects authentication only after the agent has advertised its methods.
   * Providers must never guess a private method identifier before initialize.
   */
  ...
  /**
   * Classifies bounded stderr emitted while the child is starting. The raw
   * bytes are drained regardless, but are never logged by this runtime. This
   * hook exists for providers such as Grok Build whose CLI can report that a
   * requested security policy failed and then continue in a weaker mode.
   */
  readonly classifyStartupStderr?: (boundedStderr: string) => EffectAcpErrors.AcpError | undefined;
}
...
export class AcpSessionRuntime extends Context.Service<
  AcpSessionRuntime,
  {
    ...
    /** Closes any active assistant segment, then drains through that completion event. */
    readonly finishPromptEvents: Effect.Effect<void>;
    ...
    /** Latest provider-advertised slash-command catalog. */
...
>()("cafecode/provider/acp/AcpSessionRuntime") {}
...
> =>
  Effect.gen(function* () {
    ...
    // A provider can emit updates faster than the projection can consume them.
    // Backpressure here bounds memory during long-running or pathological turns.
    const eventQueue = yield* Queue.bounded<AcpSessionRuntimeEvent>(2_048);
    const modeStateRef = yield* Ref.make<AcpSessionModeState | undefined>(undefined);
    const toolCallsRef = yield* Ref.make(new Map<string, AcpToolCallState>());
    const assistantItemRuntimeId = yield* Random.nextUUIDv4;
    ...
    const child = yield* spawner
      .spawn(
        ChildProcess.make(options.spawn.command, options.spawn.args, {
          ...
          // ACP is a structured stdio protocol. Shell expansion would make
          // custom paths and arguments command-injection surfaces.
          shell: false,
        }),
      )
      .pipe(
    ...
    // Child stderr is not part of ACP's stdout protocol, but it still has to be
    // consumed for the lifetime of the process or a chatty provider can block
    ...
    yield* acp.handleSessionUpdate((notification) =>
      Effect.gen(function* () {
        // Some agents publish their command catalog while `session/new` is
        // still resolving. Capture that state before the normal Started gate;
        // streaming content/tool updates remain gated to the bound session.
        if (notification.update.sessionUpdate === "available_commands_update") {
          const commandsEvent = parseSessionUpdateEvent(notification).events.find(
            (event) => event._tag === "AvailableCommandsChanged",
          );
          if (commandsEvent?._tag === "AvailableCommandsChanged") {
            yield* Ref.set(availableCommandsRef, commandsEvent.commands);
          }
        }
    ...
    const startOnceBase = Effect.gen(function* () {
    ...
    const startOnce = options.classifyStartupStderr
      ? Effect.raceFirst(
          startOnceBase.pipe(
            ...
            // introducing a test-clock or wall-clock startup delay.
            Effect.tap(() =>
              Effect.gen(function* () {
          ...
          Deferred.await(startupStderrFailure),
        )
      : startOnceBase;
    ...
    return {
      ...
      drainEvents,
      finishPromptEvents: closeActiveAssistantSegment({
        queue: eventQueue,
        assistantSegmentRef,
      }).pipe(Effect.andThen(drainEvents)),
      ...
      cancel: getStartedState.pipe(
        Effect.flatMap((started) =>
          Effect.gen(function* () {
            // ACP cancellation is a notification, so the only delivery
            // boundary available to the client is completion of the transport
            // write. Send it before interrupting the local prompt RPC fiber.
            // Interrupting first can remove the pending prompt and let callers
            // close the owned process before `session/cancel` ever reaches the
            // agent, which leaves provider-side tools running after Cafe has
            // reported the turn as stopped.
