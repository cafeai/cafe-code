import type { ProviderUserInputAnswers, UserInputQuestion } from "@cafecode/contracts";
...
const XAiWrappedPromptCompleteNotification = Schema.Struct({
  method: Schema.Literals(["x.ai/session/prompt_complete", "_x.ai/session/prompt_complete"]),
  params: XAiPromptCompleteNotification,
});

export const XAiPromptCompleteNotificationPayload = Schema.Union([
  XAiPromptCompleteNotification,
  XAiWrappedPromptCompleteNotification,
]);

const XAiSessionUpdateNotification = Schema.Struct({
  sessionId: Schema.String,
  // Grok multiplexes several private task and lifecycle updates through this
  // method. Decode the envelope here and strictly decode only the terminal
  // variant below so unrelated forward-compatible updates remain ignorable.
  update: Schema.Unknown,
});

const XAiTurnCompletedSessionUpdate = Schema.Struct({
...
const decodeXAiTurnCompletedSessionUpdate = Schema.decodeUnknownOption(
  XAiTurnCompletedSessionUpdate,
);

type XAiPromptCompleteNotification = typeof XAiPromptCompleteNotification.Type;
type XAiPromptCompleteNotificationPayload = typeof XAiPromptCompleteNotificationPayload.Type;

function unwrapPromptCompleteNotification(
  value: XAiPromptCompleteNotificationPayload,
): XAiPromptCompleteNotification {
  return "params" in value ? value.params : value;
}
...
export const XAiExitPlanModeParams = Schema.Struct({
  sessionId: Schema.String,
  toolCallId: Schema.String,
...
export const XAiExitPlanModeResponse = Schema.Struct({
  outcome: Schema.Literals(["approved", "cancelled", "abandoned"]),
  feedback: Schema.optional(Schema.String),
});
export type XAiExitPlanModeRequest = typeof XAiExitPlanModeRequest.Type;
export type XAiExitPlanModeResponse = typeof XAiExitPlanModeResponse.Type;

export function extractXAiExitPlanModeParams(
  request: XAiExitPlanModeRequest,
): typeof XAiExitPlanModeParams.Type {
  return "params" in request ? request.params : request;
}

export const XAiInterjectRequest = Schema.Struct({
  sessionId: Schema.String,
  text: Schema.String,
  interjectionId: Schema.optional(Schema.String),
  content: Schema.optional(Schema.Array(EffectAcpSchema.ContentBlock)),
});
export const XAiInterjectResponse = Schema.Struct({ status: Schema.Literal("queued") });
export type XAiInterjectRequest = typeof XAiInterjectRequest.Type;
export type XAiInterjectResponse = typeof XAiInterjectResponse.Type;

const XAiUsageModel = Schema.Struct({
  ...
  costUsdTicks: Schema.optional(Schema.NullOr(Schema.Number)),
  costIsPartial: Schema.optional(Schema.Boolean),
});
export const XAiSessionUsageRequest = Schema.Struct({ sessionId: Schema.String });
export const XAiSessionUsageResponse = Schema.Struct({
  usage: Schema.Struct({
    ...XAiUsageModel.fields,
    modelUsage: Schema.optional(Schema.Record(Schema.String, XAiUsageModel)),
    numTurns: Schema.Number,
    usageIsIncomplete: Schema.optional(Schema.Boolean),
  }),
});
export type XAiSessionUsageResponse = typeof XAiSessionUsageResponse.Type;

export const XAiRewindPointsRequest = Schema.Struct({ sessionId: Schema.String });
export const XAiRewindPointsResponse = Schema.Struct({
  rewind_points: Schema.Array(
    Schema.Struct({
      prompt_index: Schema.Number,
      created_at: Schema.String,
      num_file_snapshots: Schema.Number,
      has_file_changes: Schema.optional(Schema.Boolean),
      prompt_preview: Schema.optional(Schema.NullOr(Schema.String)),
    }),
  ),
});
export const XAiRewindExecuteRequest = Schema.Struct({
  sessionId: Schema.String,
  targetPromptIndex: Schema.optional(Schema.Number),
  targetResponseId: Schema.optional(Schema.String),
  force: Schema.optional(Schema.Boolean),
  mode: Schema.optional(Schema.Literals(["all", "conversation_only", "files_only"])),
});
export const XAiRewindExecuteResponse = Schema.Struct({
  success: Schema.Boolean,
  target_prompt_index: Schema.Number,
  mode: Schema.Literals(["all", "conversation_only", "files_only"]),
  reverted_files: Schema.Array(Schema.String),
  clean_files: Schema.optional(Schema.Array(Schema.String)),
  conflicts: Schema.Array(Schema.Struct({ path: Schema.String, conflict_type: Schema.String })),
  prompt_text: Schema.optional(Schema.NullOr(Schema.String)),
...
export const makeXAiPromptCompletionRuntime = Effect.fn("makeXAiPromptCompletionRuntime")(
  function* (runtime: AcpSessionRuntime.AcpSessionRuntime["Service"]) {
    ...
    const allocatePromptFallbackId = Effect.sync(() => {
      ...
      return `cafecode-xai-prompt-${nextPromptFallbackId}`;
    });

    yield* runtime.handleExtNotification(
      "_x.ai/session/prompt_complete",
      XAiPromptCompleteNotificationPayload,
      (notification) =>
        resolveXAiPromptCompletionFallback({
          pendingRef,
          completedPromptIdsRef,
          notification: unwrapPromptCompleteNotification(notification),
        }),
    );
    yield* runtime.handleExtNotification(
    ...
    const handleSessionUpdate = (notification: typeof XAiSessionUpdateNotification.Type) => {
      const update = Option.getOrUndefined(
        decodeXAiTurnCompletedSessionUpdate(notification.update),
      );
      if (update === undefined) return Effect.void;
      return resolveXAiPromptCompletionFallback({
        pendingRef,
        completedPromptIdsRef,
        notification: {
          sessionId: notification.sessionId,
          ...(update.prompt_id ? { promptId: update.prompt_id } : {}),
          ...(update.stop_reason ? { stopReason: update.stop_reason } : {}),
          ...(update.agent_result !== undefined ? { agentResult: update.agent_result } : {}),
        },
      });
    };
    // Stable Grok 1.0.4 emits completion as a private session-update variant,
    // while source builds have used the dedicated prompt_complete method.
    // Register both spellings and normalize them into the same prompt race.
    yield* runtime.handleExtNotification(
      ...
      cancel: Ref.get(activeSessionIdRef).pipe(
        Effect.flatMap((sessionId) =>
          sessionId === undefined
            ? runtime.cancel
            : runtime.cancel.pipe(
                // Preserve the provider notification as the first action. The
                // fallback exists only to release a private xAI completion
                // waiter after the standard ACP runtime has delivered cancel;
                // resolving it first can make the prompt look terminal before
                // the provider has even received `session/cancel`.
                Effect.andThen(abortPendingPromptCompletions(pendingRef, sessionId)),
              ),
        ),
      ),
