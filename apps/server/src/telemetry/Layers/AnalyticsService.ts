...
const TelemetryEnvConfig = Config.all({
  posthogKey: cafeCodeConfigWithDefault(
    "CAFE_CODE_POSTHOG_KEY",
    Config.string,
    "phc_XOWci4oZP4VvLiEyrFqkFjP4CZn55mjYYBMREK5Wd6m",
  ),
  posthogHost: cafeCodeConfigWithDefault(
    "CAFE_CODE_POSTHOG_HOST",
    Config.string,
    "https://us.i.posthog.com",
  ),
  enabled: cafeCodeConfigWithDefault("CAFE_CODE_TELEMETRY_ENABLED", Config.boolean, true),
  flushBatchSize: cafeCodeConfigWithDefault(
    "CAFE_CODE_TELEMETRY_FLUSH_BATCH_SIZE",
    Config.number,
    20,
  ),
  maxBufferedEvents: cafeCodeConfigWithDefault(
    "CAFE_CODE_TELEMETRY_MAX_BUFFERED_EVENTS",
    Config.number,
    1_000,
  ),
});
    ...
    const payload = {
      ...
      batch: events.map((event) => ({
        ...
        properties: {
          ...
          arch: process.arch,
          cafeCodeVersion: packageJson.version,
