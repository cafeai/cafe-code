...
const bitbucketPullRequest = {
  ...
  links: {
    html: {
      href: "https://bitbucket.org/cafeai/cafe-code/pull-requests/42",
    },
  },
  source: {
...
const repositoryJson = {
  full_name: "cafeai/cafe-code",
  links: {
    html: { href: "https://bitbucket.org/cafeai/cafe-code" },
    clone: [
      { name: "https", href: "https://bitbucket.org/cafeai/cafe-code.git" },
      { name: "ssh", href: "git@bitbucket.org:cafeai/cafe-code.git" },
    ],
  },
...
function makeLayer(input: {
  ...
  const gitMock = {
    readConfigValue: vi.fn<GitVcsDriver.GitVcsDriverShape["readConfigValue"]>(() =>
      Effect.succeed<string | null>("git@bitbucket.org:cafeai/cafe-code.git"),
    ),
    ...
    listRemotes: () =>
      Effect.succeed({
        remotes: [
          {
            name: "origin",
            url: "git@bitbucket.org:cafeai/cafe-code.git",
            pushUrl: Option.none(),
            isPrimary: true,
          },
        ],
    ...
    Layer.provide(
      ConfigProvider.layer(
        ConfigProvider.fromEnv({
          env: {
            CAFE_CODE_BITBUCKET_API_BASE_URL: "https://api.test.local/2.0",
            CAFE_CODE_BITBUCKET_EMAIL: "user@example.com",
            CAFE_CODE_BITBUCKET_API_TOKEN: "token",
          },
        }),
      ),
    ),
...
it.effect("parses pull request responses from the Bitbucket REST API", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(result, {
      number: 42,
      title: "Add Bitbucket provider",
      url: "https://bitbucket.org/cafeai/cafe-code/pull-requests/42",
      baseRefName: "main",
    ...
    assert.strictEqual(
      ...
      "https://api.test.local/2.0/repositories/cafeai/cafe-code/pullrequests/42",
    );
...
it.effect("lists pull requests with Bitbucket state and source branch query params", () => {
        ...
        values: [
          {
            ...
            source: {
              ...
              repository: { full_name: "cafeai/cafe-code" },
            },
          },
        ],
      }),
  ...
  return Effect.gen(function* () {
    ...
    assert.strictEqual(
      request?.url,
      "https://api.test.local/2.0/repositories/cafeai/cafe-code/pullrequests",
    );
...
it.effect("reads repository clone URLs and default branch", () => {
  ...
  return Effect.gen(function* () {
    ...
    const cloneUrls = yield* bitbucket.getRepositoryCloneUrls({
      cwd: "/repo",
      repository: "cafeai/cafe-code",
    });
    const defaultBranch = yield* bitbucket.getDefaultBranch({ cwd: "/repo" });

    assert.deepStrictEqual(cloneUrls, {
      nameWithOwner: "cafeai/cafe-code",
      url: "https://bitbucket.org/cafeai/cafe-code.git",
      sshUrl: "git@bitbucket.org:cafeai/cafe-code.git",
    });
  ...
  () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.deepStrictEqual(
        ...
        [
          "https://api.test.local/2.0/repositories/cafeai/cafe-code",
          "https://api.test.local/2.0/repositories/cafeai/cafe-code/branching-model",
        ].toSorted(),
      );
    }).pipe(Effect.provide(layer));
  },
...
it.effect("creates repositories through the Bitbucket REST API", () => {
  ...
  return Effect.gen(function* () {
    ...
    const request = execute.mock.calls[0]?.[0];
    assert.strictEqual(request?.url, "https://api.test.local/2.0/repositories/cafeai/cafe-code");
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(JSON.parse(new TextDecoder().decode(rawBody)), {
      ...
      source: {
        ...
        repository: { full_name: "owner/cafe-code" },
      },
      destination: {
        branch: { name: "main" },
      },
  ...
  return Effect.gen(function* () {
    ...
    yield* bitbucket.checkoutPullRequest({
      ...
      context: {
        ...
        remoteName: "origin",
        remoteUrl: "git@bitbucket.org:cafeai/cafe-code.git",
      },
      reference: "42",
      force: true,
    });
...
it.effect("checks out fork pull requests through an ensured fork remote", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(git.fetchRemoteBranch.mock.calls[0]?.[0], {
      ...
      remoteName: "octocat",
      remoteBranch: "main",
      localBranch: "cafecode/pr-42/main",
    });
    assert.deepStrictEqual(git.setBranchUpstream.mock.calls[0]?.[0], {
      cwd: "/repo",
      branch: "cafecode/pr-42/main",
      remoteName: "octocat",
      remoteBranch: "main",
    });
    assert.deepStrictEqual(git.switchRef.mock.calls[0]?.[0], {
      cwd: "/repo",
      refName: "cafecode/pr-42/main",
    });
  }).pipe(Effect.provide(layer));
});
