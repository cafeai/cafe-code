...
const SavedEnvironmentRegistryDocumentSchema = Schema.Struct({
  ...
  records: Schema.optionalKey(Schema.Array(Schema.Unknown)),
});
const decodePersistedSavedEnvironmentStorageRecord = Schema.decodeUnknownOption(
  PersistedSavedEnvironmentStorageRecordSchema,
);
...
>()("cafecode/desktop/SavedEnvironments") {}
...
function normalizeSavedEnvironmentRegistryDocument(
  ...
  return {
    ...
    records: (document.records ?? []).flatMap((record) =>
      Option.match(decodePersistedSavedEnvironmentStorageRecord(record), {
        onNone: () => [],
...
const writeRegistryDocument = Effect.fn("desktop.savedEnvironments.writeRegistryDocument")(
  function* (input: {
    ...
    yield* input.fileSystem.chmod(input.registryPath, 0o600);
  },
);
