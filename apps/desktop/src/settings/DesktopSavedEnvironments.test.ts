...
const encodeSavedEnvironmentRegistryDocumentProbe = Schema.encodeSync(
  Schema.fromJsonString(SavedEnvironmentRegistryDocumentProbe),
);
...
describe("DesktopSavedEnvironments", () => {
  ...
  it.effect("keeps valid records when another stored record is malformed", () =>
    withSavedEnvironments(
      Effect.gen(function* () {
        const environment = yield* DesktopEnvironment.DesktopEnvironment;
        const fileSystem = yield* FileSystem.FileSystem;
        ...
        yield* fileSystem.writeFileString(
          ...
          encodeSavedEnvironmentRegistryDocumentProbe({
            version: 1,
            records: [savedRegistryRecord, { environmentId: "malformed-record" }],
          }),
        );
