...
const CURRENT_SCHEMA_COMMIT = "e87bde7322ceb8d52d9e81718f5783beb66f9f6d";
const EXPECTED_SCHEMA_SHA256 = "1ceeff8f64fd750688f98ea13099436a908012866f2343698f6a9ac85a652bb0";
const EXPECTED_META_SHA256 = "ebdaf23edddbd6865e41e4d233136c56573344d6cd657d41144fe32dacc6dbaa";
...
const downloadFile = Effect.fn("downloadFile")(function* (
  ...
  if (actualSha256 !== expectedSha256) {
    return yield* Effect.fail<GenerateCommandError>({
      ...
      message: `ACP release asset digest mismatch for ${url}. Expected ${expectedSha256}, received ${actualSha256}.`,
    });
  }
...
const downloadSchemas = Effect.fn("downloadSchemas")(function* (tag: string) {
  ...
  yield* downloadFile(
    `${baseUrl}/schema.unstable.json`,
    upstreamSchemaPath,
    EXPECTED_SCHEMA_SHA256,
  );
  yield* downloadFile(`${baseUrl}/meta.unstable.json`, upstreamMetaPath, EXPECTED_META_SHA256);
...
const generateSchemas = Effect.fn("generateSchemas")(function* (skipDownload: boolean) {
  ...
  const prelude = [
    ...
    `// ACP schema release: ${CURRENT_SCHEMA_RELEASE} (${CURRENT_SCHEMA_COMMIT})`,
    `// schema.unstable.json sha256: ${EXPECTED_SCHEMA_SHA256}`,
    `// meta.unstable.json sha256: ${EXPECTED_META_SHA256}`,
    "",
  ];
