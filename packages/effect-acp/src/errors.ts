...
export class AcpRequestError extends Schema.TaggedErrorClass<AcpRequestError>()("AcpRequestError", {
  ...
  static fromProtocolError(
    ...
    return new AcpRequestError({
      code: error.code,
      // Agent-supplied JSON-RPC messages and data are not trustworthy
      // diagnostics: providers can echo prompts, tool arguments, paths, or
      // credentials in either field. Preserve only the structural error code
      // and Cafe-owned request context across the protocol boundary.
      errorMessage: `ACP method '${context.method}' failed with code ${error.code}.`,
  ...
  static fromCoreHandlerError(error: AcpError, method: string): AcpRequestError {
  ...
  static fromExtensionHandlerError(error: AcpError, method: string): AcpRequestError {
    if (error._tag === "AcpRequestError") {
      return error;
    }
    return AcpRequestError.internalError(
