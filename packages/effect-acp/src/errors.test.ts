...
describe("effect-acp errors", () => {
  ...
  it.effect("preserves structural protocol request facts without agent-supplied detail", () => {
    const secret = "provider-error-secret";
    const failure = AcpSchema.Error.make({
      code: -32602,
      message: `Invalid params: ${secret}`,
      data: { field: "sessionId", secret },
    });
    ...
    return Effect.gen(function* () {
      ...
      expect(error).toMatchObject({
        ...
        code: -32602,
        errorMessage: "ACP method 'session/load' failed with code -32602.",
        method: "session/load",
        operation: "receive-response",
      });
      if (error._tag !== "AcpRequestError") {
        throw new Error(`Expected AcpRequestError, received ${error._tag}`);
      }
      expect(error.data).toBeUndefined();
      expect(encodeUnknownJson(error)).not.toContain(secret);
    });
  });
