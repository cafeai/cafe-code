...
import {
  ...
  buildRemoteCafeCodeRunnerScript,
...
describe("ssh tunnel scripts", () => {
  it("builds the remote Cafe Code runner with npx and npm fallbacks", () => {
    const script = buildRemoteCafeCodeRunnerScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE });
    ...
    assert.include(script, 'for CAFE_CODE_NODE_BIN in "$NVM_DIR"/versions/node/*/bin');
  ...
  it("does not hard-code a remote node engine range", () => {
    ...
    assert.include(script, "CAFE_CODE_NODE_ENGINE_RANGE=''");
  ...
  it("builds the remote Cafe Code runner with a node script override", () => {
    ...
    assert.include(
      script,
      "CAFE_CODE_NODE_SCRIPT_PATH='/Users/julius/Development/Work/codething-mvp/apps/server/dist/bin.mjs'",
    );
    assert.include(script, 'exec node "$CAFE_CODE_NODE_SCRIPT_PATH" "$@"');
  });

  it("uses the remote Cafe Code runner for launch and pairing scripts", () => {
    const target = {
      alias: "devbox",
