...
describe("extractPathFromShellOutput", () => {
  it("extracts the path between capture markers", () => {
    expect(
      extractPathFromShellOutput(
        "__CAFE_CODE_PATH_START__\n/opt/homebrew/bin:/usr/bin\n__CAFE_CODE_PATH_END__\n",
      ),
...
describe("readPathFromLoginShell", () => {
  it("uses a shell-agnostic printenv PATH probe", () => {
    ...
    >(() => "__CAFE_CODE_ENV_PATH_START__\n/a:/b\n__CAFE_CODE_ENV_PATH_END__\n");
    ...
    expect(args?.[1]).toContain("__CAFE_CODE_ENV_PATH_START__");
    expect(args?.[1]).toContain("__CAFE_CODE_ENV_PATH_END__");
...
describe("readEnvironmentFromLoginShell", () => {
  it("extracts multiple environment variables from a login shell command", () => {
    ...
    >(() =>
      [
        ...
        "/a:/b",
        "__CAFE_CODE_ENV_PATH_END__",
        "__CAFE_CODE_ENV_SSH_AUTH_SOCK_START__",
        "/tmp/secretive.sock",
        "__CAFE_CODE_ENV_SSH_AUTH_SOCK_END__",
      ].join("\n"),
    );
  ...
  it("preserves surrounding whitespace in captured values", () => {
    ...
    >(() =>
      [
        "__CAFE_CODE_ENV_CUSTOM_VAR_START__",
        "  padded value  ",
        "__CAFE_CODE_ENV_CUSTOM_VAR_END__",
      ].join("\n"),
    );
...
describe("readEnvironmentFromWindowsShell", () => {
  ...
  it("strips CRLF delimiters from captured PowerShell values", () => {
    ...
    >(
      () =>
        "__CAFE_CODE_ENV_FNM_DIR_START__\r\nC:\\Users\\testuser\\AppData\\Roaming\\fnm\r\n__CAFE_CODE_ENV_FNM_DIR_END__\r\n",
    );
  ...
  it("falls back to Windows PowerShell when pwsh.exe is unavailable", () => {
    ...
    >((file) => {
      ...
      return "__CAFE_CODE_ENV_PATH_START__\nC:\\Tools\n__CAFE_CODE_ENV_PATH_END__\n";
    });
