...
Cafe Code has one server-side observability model:
...
Completed spans are written as NDJSON records to `serverTracePath` (by default, `~/.cafecode/userdata/logs/server.trace.ndjson`).

Existing `~/.t3` trace locations remain supported as a legacy fallback when no
Cafe Code home directory has been created yet.
...
```bash
export CAFE_CODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export CAFE_CODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export CAFE_CODE_OTLP_SERVICE_NAME=cafe-code-local
```

Optional:

```bash
export CAFE_CODE_TRACE_MIN_LEVEL=Info
export CAFE_CODE_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `CAFE_CODE_OTLP_*`.

macOS app bundle example:

```bash
CAFE_CODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
CAFE_CODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
CAFE_CODE_OTLP_SERVICE_NAME=cafe-code-desktop \
...
```bash
tail -f "$CAFE_CODE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -c 'select(.exit._tag != "Success") | {
  ...
  exit,
  attributes
}' "$CAFE_CODE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -r 'select(.traceId == "TRACE_ID_HERE") | [
  ...
  (.parentSpanId // "-"),
  .durationMs
] | @tsv' "$CAFE_CODE_HOME/userdata/logs/server.trace.ndjson"
```
...
- service name such as `cafe-code-local`, `t3-dev`, or `cafe-code-desktop`
...
- `CAFE_CODE_OTLP_TRACES_URL` was not set
...
Cafe Code reads `CAFE_CODE_*` environment variables.

Local trace file:

- `CAFE_CODE_TRACE_FILE`: override trace file path
- `CAFE_CODE_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `CAFE_CODE_TRACE_MAX_FILES`: rotated file count, default `10`
- `CAFE_CODE_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `CAFE_CODE_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `CAFE_CODE_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `CAFE_CODE_OTLP_TRACES_URL`: OTLP trace endpoint
- `CAFE_CODE_OTLP_METRICS_URL`: OTLP metric endpoint
- `CAFE_CODE_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `CAFE_CODE_OTLP_SERVICE_NAME`: service name, default `cafe-code-server`
