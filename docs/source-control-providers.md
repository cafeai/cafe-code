...
Cafe Code connects directly to your Git hosting provider so you can create pull requests, review code, and manage repositories without leaving your editor. Work stays in flow—no more jumping between browser tabs and terminal windows.

## Supported Providers

Cafe Code works with the platforms your team already uses:
...
- Cafe Code can suggest titles and descriptions based on your commits
...
1. Install the GitHub CLI on the machine running Cafe Code:
   ```bash
   brew install gh
   ```
2. Sign in:
   ...
   gh auth login
   ```
3. Open **Settings → Source Control** in Cafe Code and verify GitHub shows as authenticated
...
2. Add these environment variables to the environment running Cafe Code:
   ```bash
   export CAFE_CODE_BITBUCKET_EMAIL="you@example.com"
   export CAFE_CODE_BITBUCKET_API_TOKEN="your-token"
   ```
3. Restart Cafe Code and verify the connection in **Source Control settings**.
...
**Git is required** – Cafe Code uses Git for all local operations. Ensure `git` is installed on your server.

**Server-side setup** – Authentication happens on the machine running Cafe Code (the server), not your local browser. If you're using a hosted or team instance, your administrator may have already configured providers.
