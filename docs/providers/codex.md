...
This guide is for people who want to use more than one Codex account in Cafe Code.
...
In Cafe Code Settings, name it something obvious:
...
In Cafe Code Settings, add another Codex provider:
...
Cafe Code shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Cafe Code treats it
